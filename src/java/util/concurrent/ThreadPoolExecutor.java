...
public class ThreadPoolExecutor extends AbstractExecutorService {
    ...
    public void execute(Runnable command) {
         ...
         * 2.【R】添加失败（由于线程争用，线程池状态发生了变化） TODO 个人觉得不可能,出现争用,循环中会重复执行
         * 3.【R】启动失败（少见）
         *
         * 此时，会尝试将任务添加到阻塞队列
         */
        ...
        if (isRunning(state) && workQueue.offer(command)) {
             ...
             *
             * 情形2：
             * 这种情形下，可能是阻塞队列满了，此时改用【最大阈值】做为线程池的容量限制，
    ...
    private boolean addWorker(Runnable firstTask, boolean core) {
        retry:
        for (int state = ctl.get(); ; ) { // --> 获取线程池状态后,死循环
            /* Check if queue empty only if necessary */

            // {state:0123} 如果线程池不再接收新线程
            ...
            for (; ; ) {
                ...
                /* 至此，说明上面的原子操作失败了，可能是发生了线程争用(线程池状态变化或者线程池中线程数量变化) */
    ...
    private Runnable getTask() {
        ...
        for (; ; ) {
            ...
            if (runStateAtLeast(state, SHUTDOWN)) {
                ...
                // {123} 如果线程池不可以处理阻塞队列中的阻塞任务了，或者{0}可以处理，但线程池为空
    ...
    final void tryTerminate() {
        for (; ; ) {
            ...
            /*
             * 至此，有两种可能：
             * 1.线程池处于{1}【停止】状态（不接收新线程，也不再处理阻塞任务，且正在处理的阻塞任务也要被中断）
             * 2.线程池处于{0}【关闭】状态（还可以处理阻塞队列的阻塞任务），但阻塞队列为空（没有阻塞任务了）
             */
     ...
     *
     * onlyOne==true ：要求至少给一个线程被设置了中断标记
