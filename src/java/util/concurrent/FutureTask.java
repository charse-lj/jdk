...
public class FutureTask<V> implements RunnableFuture<V> {
     ...
     * 0:新建任务 --> 中间状态
     * 1:任务已结束 --> 中间状态
     * 2:任务正常结束，并设置了返回值 --> 最终状态
     * 3:任务异常结束，并设置了异常信息 --> 最终状态
     * 4:任务已取消 --> 最终状态
     * 5:任务正在中断 --> 最终状态
     * 6:任务已中断 --> 最终状态
     */
    ...
    public void run() {
        ...
        try {
            ...
            if (task != null && state == NEW) {
                ...
                boolean suc;
                try {
                    // 执行任务，并等待获取计算结果 -->并不是所有方法的执行,都是立刻有返回值
                    result = task.call();
                    suc = true;
                } catch (Throwable ex) {
                    result = null;
    ...
    /**
     * 中止异步任务，包括取消或中断
     * @param mayInterruptIfRunning 指示是否可在任务执行期间中断线程
     */
    @Override
        ...
        for (WaitNode q; (q = waiters) != null; ) {
            ...
            if (WAITERS.weakCompareAndSet(this, q, null)) {
                ...
                for (; ; ) {
                    ...
                    // unlink to help gc,帮助回收
                    q.next = null;
                    //当前遍历的元素 变成 当前元素的下一个元素 -->进行循环
                    q = next;
                }
    ...
    private int awaitDone(boolean timed, long nanos) throws InterruptedException {
        ...
        for (; ; ) {
            ...
            if (s > COMPLETING) {
                ...
                // 测试当前调用get()方法的线程是否已经中断，线程的中断状态会被清除
        ...
        if (node != null) {
            ...
            for (; ; ) {
                ...
                for (WaitNode pred = null, q = waiters, s; q != null; q = s) {
                    ...
                    //q -->遍历链表中的当前元素
                    if (q.thread != null) {
                        //遍历过程中,暂存q.thread 不为null的最后一个元素
                        pred = q;
                    } else if (pred != null) {
                        // 移除等待栈中失效的等待结点
                        // 进入这里,q.thread ==null,需要移除该元素
                        pred.next = s;
